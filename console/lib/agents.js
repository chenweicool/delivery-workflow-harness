...
const AGENT_CONTRACTS = {
  'requirement-analyst': {
    ...
    requiredOutputs: ['prd/document.md', 'design/process/context-summary.md', 'design/process/requirement-confirmation.md'],
  },
  'technical-designer': {
    name: '技术方案 Agent',
  ...
  'coding-implementer': {
    ...
    requiredOutputs: ['tasks/process/task-progress.md', 'review/process/change-log.md', 'review/process/self-check.md'],
  },
  reviewer: {
    name: 'Review Agent',
    role: '围绕当前 diff、方案和任务记录做质量审查，输出风险和修复建议。',
    sessionPolicy: '建议使用独立会话，降低实现阶段上下文偏见。',
    requiredOutputs: ['review/quality-report.md', 'review/evidence/risk-list.md'],
  },
  tester: {
    name: '测试 Agent',
    role: '生成测试策略、单测建议和执行记录，补齐质量门禁证据。',
    sessionPolicy: '可接续 Review 会话，也可独立执行。',
    requiredOutputs: ['review/evidence/unit-test-result.md', 'review/evidence/traceability-matrix.md', 'review/evidence/smoke-test-result.md'],
  },
  archivist: {
    name: '归档 Agent',
