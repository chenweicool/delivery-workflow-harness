...
- 如可用，优先参考 `api-doc-generation` 做接口文档规划，参考 `java-unit-test` 做测试策略建议，参考 `code-review` 补充风险分类。
- 如果推荐 skill 可用，可以使用其中的领域检查清单完善技术方案，但本阶段不得执行代码修改。
- 如果 skill 不可用，继续按本命令文件执行，并把缺失的 skill 相关工作记录到研发协作方案或待确认事项。
- 输出必须是中文。
...
| 状态 | 编号 | 来源问题 | 确认项 | 影响范围 | AI 推荐结论 | 人工确认结果 | 证据 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 待确认 | DEV-001 | 分支策略未确认 | 基准分支 | 代码实现 | 根据当前应用默认主干或用户指定分支确认 | 待人工补充 |  |
| 待确认 | DEV-002 | 分支策略未确认 | 开发分支命名 | 代码实现 | 使用 `codex/<需求英文简称>` 或团队约定格式 | 待人工补充 |  |
