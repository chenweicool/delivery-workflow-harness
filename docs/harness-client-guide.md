...
- 报告接收地址和授权页地址由每台电脑在本机配置保存；浏览器回调始终是运行时创建的 `127.0.0.1` 临时端口，不写入源码。
- `npm run check` 会扫描 `bin/`、`console/` 和 `scripts/` 中的 JavaScript；检测到 `10.x.x.x`、`172.16-31.x.x` 或 `192.168.x.x` 私网地址即失败，避免测试代码带出内网地址。
